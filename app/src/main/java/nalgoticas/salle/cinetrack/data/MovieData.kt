package nalgoticas.salle.cinetrack.data

data class Movie(
    val id: Int,
    val title: String,
    val year: Int,
    val durationMinutes: Int,
    val genres: List<String>,
    val rating: Float,
    val imageUrl: String,
    val synopsis: String,
    val director: String,
    val cast: String
)

object MovieData {

    val movies = listOf(
        Movie(
            id = 1,
            title = "Parasite",
            year = 2019,
            durationMinutes = 132,
            genres = listOf("Drama", "Thriller"),
            rating = 4.7f,
            imageUrl = "https://image.tmdb.org/t/p/w500/7IiTTgloJzvGI1TAYymCfbfl3vT.jpg",
            synopsis = "Greed and class discrimination threaten the newly formed symbiotic relationship between the wealthy Park family and the destitute Kim clan.",
            director = "Bong Joon-ho",
            cast = "Song Kang-ho, Lee Sun-kyun, Cho Yeo-jeong, Choi Woo-shik"
        ),
        Movie(
            id = 2,
            title = "Interstellar",
            year = 2014,
            durationMinutes = 169,
            genres = listOf("Adventure", "Sci-Fi"),
            rating = 4.5f,
            imageUrl = "https://image.tmdb.org/t/p/w500/rAiYTfKGqDCRIIqo664sY9XZIvQ.jpg",
            synopsis = "A team of explorers travel through a wormhole in space in an attempt to ensure humanity's survival.",
            director = "Christopher Nolan",
            cast = "Matthew McConaughey, Anne Hathaway, Jessica Chastain"
        ),
        Movie(
            id = 3,
            title = "Inception",
            year = 2010,
            durationMinutes = 148,
            genres = listOf("Action", "Sci-Fi"),
            rating = 4.6f,
            imageUrl = "https://image.tmdb.org/t/p/w500/edv5CZvWj09upOsy2Y6IwDhK8bt.jpg",
            synopsis = "A thief who steals corporate secrets through the use of dream-sharing technology is given the inverse task of planting an idea.",
            director = "Christopher Nolan",
            cast = "Leonardo DiCaprio, Joseph Gordon-Levitt, Ellen Page"
        ),
        Movie(
            id = 4,
            title = "The Dark Knight",
            year = 2008,
            durationMinutes = 152,
            genres = listOf("Action", "Crime"),
            rating = 4.7f,
            imageUrl = "https://image.tmdb.org/t/p/w500/qJ2tW6WMUDux911r6m7haRef0WH.jpg",
            synopsis = "Batman sets out to dismantle the remaining criminal organizations that plague Gotham, but finds himself prey to a reign of chaos unleashed by the Joker.",
            director = "Christopher Nolan",
            cast = "Christian Bale, Heath Ledger, Aaron Eckhart"
        )
    )

    fun getMovie(id: Int): Movie? =
        movies.firstOrNull { it.id == id }
}
